...
(2)To run this pipeline, we should run the script 'bam_read.py' to read the fragments for all the samples (bam files) and write it to txt files.  
For example:python bam_read.py -in C309.filter.bam -out PNAS/C309 could be used to write all the fragment information into the path PNAS/C309. We suggest puyting all the samples from the same data set into the same folder. For example, when we analysis the Sun et. al. data set, we should put the fragment information of all the samples in the folder PNAS.
...
file_list: the file name of an excel (.xlsx or xls) file, which contain all the sample names in the category, or a cell variable (N*1) contain all the sample names. For example, 'HCC.xlsx', which contain one column, in each row of the column is the name an HCC sample.
...
enrichment:whether or not do enrichment for the hotspots:  argument: 0 (not do) or 1 (do enrichment), default: 1.
An example to run the pipeline is: Hotspot_call_multi_sample('HCC.xlsx','PNAS','HCC',1,'enrichment',0);
After running the function, the program will produce a folder 'HCC' in the current path, and in HCC/result_n/, four files would exist (peak_all.mat,peaks.bed) and two files of the fragmentation pattern around hotspots (IFS_plot.fig, IFS_plot.pdf)) in HCC/result_n/.
...
After that, in the current workplace, there would two figures (clustering_result.fig and clustering_result.pdf) and one .mat file contains the matrix that was used to do clustering analysis and the category information of these samples.
...
After that, in the current workplace, there would two figures (PCA_plot.fig and PCA_plot.pdf) and one .mat file contains the sccors of all the samples after PCA analysis and the category information of these samples.
...
Options:more matrix files and the coressponding color.
...
ANVOA_calculate (0.01,'Breast/norm_IFS.mat','m','Lung/norm_IFS.mat','k','Ovarian/norm_IFS.mat','b','healthy/norm_IFS.mat','r');
...
%%%%%%%%Use TSNE to visualize the samples from the matrix file after anvoa analysis.
matrix file: The file should including at least three variable:
data:N*M matrix,N - hotspots number,M - sample number;
label:M*1: The category of the M samples (1,2,3,etc)
p：N*1：p-value of the hotspots in anvoa test
optional:col:X*1,cell of string, describes the color of the X types, if not existed, the color will be randomly assigned.
...
e.g. Using cluster_analysis_ANVOA(1,0.01,5000,'ANVOA_IFS_matrix.mat'), based on the anvoa filtered matrix, p-value <= 0.01 will be used, and for each category of the data,
...
After running this comand, in the current workplace, there would two figures (clustering_result.fig and clustering_result.pdf) and one .mat file contains the matrix that was used to do clustering analysis and the category information of these samples (cluster_anvoa_data.mat).
